...
function saveSetKyra() {
  ...
  var emptyRow = findFirstEmptyCell('Ky Log', 'B');
  ...
  if (currentExercise == 'A') {
    ...
    SpreadsheetApp.getUi().alert(
      'error check that cell C3 in Ky Dashboard still has a value'
    );
    return -1;
  }
...
function findFirstEmptyCell(sourceSheetName, column) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
